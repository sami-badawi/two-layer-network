module ThreeLayerNetwork where

-- Three layer network in one file
...
runMain = do
  ...
  let -- generate statistics and plots for each epoch
      stats model learningRate trainingLosses evaluationLosses learningRates options = do
        ...
        plot (networkName ++ "_plot.html") (infer' model) trainingLosses evaluationLosses learningRates evaluationData
